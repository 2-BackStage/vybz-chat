...
- **MongoDB Change Stream**: 메시지 컬렉션의 변경사항을 실시간 감지하여 새로운 메시지를 즉시 전송
- **WebFlux Flux**: 비동기 스트림으로 클라이언트에 실시간 메시지 전송
- **Redis**: 채팅방 참가자 세션 및 온라인 상태 관리
- **Sink**:  
  - 일반 메시지는 Change Stream을 통해 실시간 전송  
  - **Sink는 이전 메시지 조회 시 읽음 처리된 메시지를 즉시 클라이언트에 전송**하는 별도 스트림으로 사용됨
- **Kafka**: 메시지/이벤트 브로드캐스트

### 실시간 메시지 처리 구조

1. **Change Stream**: 새로운 메시지(입력/수정) 실시간 감지 및 전송
2. **Sink**:  
   - 사용자가 이전 메시지를 조회할 때, 읽음 처리된 메시지를 즉시 프론트엔드로 전송  
   - 일반 실시간 메시지 전송에는 사용되지 않음
3. **Ping**: 연결 상태 확인(5초마다)
4. **Flux.merge**: 위 3개 스트림을 하나로 합쳐서 클라이언트에 전송

> 실시간 메시지는 MongoDB Change Stream을 통해 감지되어 바로 전송됩니다.  
> 사용자가 이전 메시지를 조회할 때, 읽음 처리된 메시지는 별도의 Sink 스트림을 통해 즉시 클라이언트로 전송됩니다.  
> 이 두 스트림(실시간/읽음처리)과 연결 상태 확인용 ping 스트림을 합쳐 하나의 SSE 스트림으로 제공합니다.
